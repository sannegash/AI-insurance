...
from claim_model import ClaimModel  # Ensure this is defined
...
def predict_risk_and_claim(new_data, scaler, input_dim, risk_model_path, claim_model_path):
    ...
    claim_model = ClaimModel(input_dim)  # Ensure ClaimModel is defined similarly to RiskModel
    claim_model.load_state_dict(torch.load(claim_model_path))
    claim_model.eval()

    # Preprocess new data
    categorical_features = ['vehicle', 'education']  # Update to match training phase
...
if __name__ == "__main__":
    ...
    scaler.feature_names_in_ = [
        'age', 'bmi', 'income', 'married', 'children', 'traffic_violations', 
        'number_of_accidents', 'vehicle_SUV', 'education_Master\'s'
